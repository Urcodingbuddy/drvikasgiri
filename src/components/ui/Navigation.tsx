    ...
    >
      <div className="mx-4 mt-4">
        {!isMobile ? (
          <div
            ref={containerRef}
            className="relative max-w-6xl mx-auto flex items-center justify-between rounded-full border border-white/10 bg-black/90 px-2 py-1.5 shadow-2xl backdrop-blur-sm pointer-events-auto"
          >
            <div
            ...
            <div className="relative z-10 flex items-center gap-1">
              {navLinks.slice(0, 4).map((link) => (
                <button
                  key={link}
                  ref={(el) => {
                    buttonRefs.current[link] = el;
                  }}
                  onClick={() => handleNav(link)}
                  className={`relative rounded-full px-6 py-2.5 text-md  transition-all duration-300 ${active === link ? "text-[#141414]" : "text-gray-300 hover:text-white"}`}
                >
                  {link}
                </button>
              ))}
          ...
          <>
            <div className="flex items-center justify-between rounded-full border border-white/10 bg-black/90 px-4 py-2.5 shadow-2xl backdrop-blur-sm pointer-events-auto">
              ...
              >
                <span
                  className={`block h-0.5 w-5 bg-white rounded-full transition-all duration-300 ${menuOpen ? "translate-y-2 rotate-45" : ""}`}
                />
                <span
                  className={`block h-0.5 w-5 bg-white rounded-full transition-all duration-300 ${menuOpen ? "opacity-0" : ""}`}
                />
                <span
                  className={`block h-0.5 w-5 bg-white rounded-full transition-all duration-300 ${menuOpen ? "-translate-y-2 -rotate-45" : ""}`}
                />
            ...
            >
              <div className="px-3 pt-3 pb-1">
                <p className="text-[9px]  uppercase tracking-[0.28em] text-primary/70 px-2 mb-2">
                  Navigation
                </p>
                  ...
                  {navLinks.map((link, index) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span
                          className={`text-[10px]  tabular-nums w-4 ${active === link ? "text-[#141414]/50" : "text-primary/60"}`}
                        >
                ...
                >
                  ...
                  <span className="flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-full bg-black/80">
                    <svg
                      ...
                      className="size-[15px] transition-transform duration-200 group-hover:-translate-y-0.5 group-hover:translate-x-0.5"
                      viewBox="0 -960 960 960"
                      fill="white"
                    >
                      <path d="m256-240-56-56 384-384H240v-80h480v480h-80v-344L256-240Z" />
