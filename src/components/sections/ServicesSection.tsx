const MI = ({ d, className }: { d: string; className?: string }) => (
...
const icons = {
  ...
  child_care:        'M11.5 2C6.81 2 3 5.81 3 10.5S6.81 19 11.5 19h.5v3c4.86-2.34 8-7 8-11.5C20 5.81 16.19 2 11.5 2zm1 14.5h-2v-2h2v2zm0-4h-2c0-3.25 3-3 3-5 0-1.1-.9-2-2-2s-2 .9-2 2h-2c0-2.21 1.79-4 4-4s4 1.79 4 4c0 2.5-3 2.75-3 5z',
...
const core = [
  { icon: icons.auto_awesome,  label: '01', title: 'Veneers & Smile Makeovers',                  desc: 'Customized, natural-looking smile transformations tailored to each individual with refined aesthetic precision.',   tag: 'Signature' },
  { icon: icons.light_mode,    label: '02', title: 'Teeth Whitening & Cosmetic Enhancements',    desc: 'Aesthetic improvements designed to create brighter, confident, naturally polished smiles.',                          tag: 'Popular' },
];

const advanced = [
  { icon: icons.medical_services,  title: 'Endodontics',             sub: 'Root Canal Treatment', desc: 'Pain-free, precision-based procedures.' },
  { icon: icons.bolt,              title: 'Laser Dentistry',          sub: null,                   desc: 'Minimally invasive, modern dental treatments.' },
  { icon: icons.health_and_safety, title: 'General & Preventive Care',sub: null,                   desc: 'Routine checkups and long-term dental maintenance.' },
  { icon: icons.child_care,        title: 'Pediatric Dentistry',      sub: null,                   desc: 'Gentle, child-friendly dental care.' },
  { icon: icons.event_available,   title: 'Preventive Dental Care',   sub: null,                   desc: 'Routine checkups and preventive maintenance.' },
  { icon: icons.content_cut,       title: 'Oral Surgery',             sub: 'Specialized',          desc: 'Treatment of complex dental and surgical conditions.' },
  { icon: icons.spa,               title: 'Periodontics',             sub: 'Specialized',          desc: 'Gum health and treatment of periodontal diseases.' },
];
  ...
  return (
    <section className="bg-[var(--color-surface-lowest)] py-16 md:py-24 overflow-hidden">
      <div className="max-w-6xl mx-auto px-4 md:px-6">
        ...
        {/* ── Header ── */}
        <div data-reveal className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-12 md:mb-16">
          <div>
        ...
        {/* ── Core Services ── */}
        <div className="mb-10">
          <p className="text-[9px] font-bold uppercase tracking-[0.3em] text-primary/50 mb-4 pl-1">Core Services</p>
                ...
                <div className="relative flex flex-col h-full gap-5">
                  <div className="flex items-start justify-between">
                    <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-primary/12 border border-primary/15">
                      <MI d={icon} className="size-5 text-primary" />
                    </div>
                    <span className="rounded-full border border-primary/25 bg-primary/8 px-3 py-1 text-[9px] font-bold uppercase tracking-[0.22em] text-primary">
                      {tag}
                    </span>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold text-primary/40 tracking-widest mb-1.5">{label}</p>
                    <h3 className="text-base md:text-lg font-bold text-white leading-snug tracking-tight mb-2">{title}</h3>
        ...
        {/* ── Advanced Treatments ── */}
        <div className="mb-4">
          <p className="text-[9px] font-bold uppercase tracking-[0.3em] text-primary/50 mb-4 pl-1">Advanced Dental Treatments</p>
            ...
            {advanced.map(({ icon, title, sub, desc }, i) => (
              ...
              >
                <div className="shrink-0 flex h-9 w-9 items-center justify-center rounded-xl bg-primary/10 group-hover:bg-primary/15 transition-colors duration-200">
                  <MI d={icon} className="size-4 text-primary" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-sm font-semibold text-white leading-snug">{title}</p>
                    {sub && (
                      <span className="text-[9px] font-bold uppercase tracking-[0.18em] text-primary/60 border border-primary/20 rounded-full px-2 py-0.5">
                        {sub}
                      </span>
                    )}
                  </div>
        ...
        >
          <p className="text-[9px] font-bold uppercase tracking-[0.28em] text-primary/50 sm:w-40 shrink-0">
            Our Promise
          </p>
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-0 sm:divide-x sm:divide-white/8 w-full">
            {highlights.map((h) => (
              <div key={h} className="flex items-center gap-2.5 sm:px-6 first:sm:pl-0">
                <MI d={icons.check_circle} className="size-3.5 text-primary shrink-0" />
